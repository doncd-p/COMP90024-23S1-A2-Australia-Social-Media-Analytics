<template>
  <div class="container" style="height: 95vh">
      ...
      <el-col :span="12">
        ...
        <el-radio-group v-model="senario" class="radio-container">
          ...
          <el-radio-button label="status"
            >Changed Governing Party</el-radio-button
          >
        </el-radio-group>
      </el-col>
      <el-col :span="4">
    ...
    <div
      ...
      id="map"
      v-loading="loading"
      style="height: 65vh; margin-bottom: 0"
      element-loading-text="loading..."
      element-loading-spinner="el-icon-loading"
      element-loading-background="rgba(0, 0, 0, 0.8)"
    ></div>
    <div style="color: white; height: 5vh; margin-bottom: 0">
      Note: Percentage of votes won is votes according to Two Candidate
      Preferences.
    </div>
    ...
    >
      <el-form label-position="left">
        ...
        <div>
          ...
          <pie-data :pieData="mapItem.sentiment"> </pie-data>
        </div>
        <div>
          <el-divider>Weekly Sentiment</el-divider>
          <line-data :lineData="mapItem.lineData"> </line-data>
          <div style="font-size: 12px; display: flex; justify-content: center">
            Note: "-5" on xAxis refers “the 5th week before the election day",
            "5" on xAxis refers "the 5th week after the election day".
          </div>
        </div>
      </el-form>

      <el-divider>Top Tweets</el-divider>
      <div style="font-weight: bolder">Top 5 Positive:</div>
      <div
        class="tweets"
        v-for="item in mapItem.posTweets"
        :key="item.tweet_id"
      >
      ...
      <br />
      <div style="font-weight: bolder">Top 5 Negative:</div>
      <div
        class="tweets"
        v-for="item in mapItem.negTweets"
        :key="item.tweet_id"
      >
        <div style="font-weight: bold; margin-bottom: 2px">
          &nbsp;{{ item.author }}:
...
export default {
  ...
  methods: {
    ...
    getCloudData() {
      ...
      if (this.time == "pre") {
        url =
          ...
          "/political/sentiments/daily?startdate=2022-02-09&enddate=2022-05-21";
      } else {
        url =
          process.env.VUE_APP_BASE_URL +
          "/political/sentiments/daily?startdate=2022-05-22&enddate=2023-06-30";
      }
    ...
    async getDetail(eName) {
      const posSrc =
        process.env.VUE_APP_BASE_URL +
        "/tweet/top_positive?num=5&electorate=" +
        eName;
      const negSrc =
        process.env.VUE_APP_BASE_URL +
        "/tweet/top_negative?num=5&electorate=" +
        eName;
      let sentimentSrc;
      let lineSrc;
      if (this.time == "pre") {
        sentimentSrc =
          process.env.VUE_APP_BASE_URL +
          "/political/sentiments/number?startdate=2022-02-09&enddate=2022-05-21&electorate=" +
          eName;
        lineSrc =
          process.env.VUE_APP_BASE_URL +
          "/political/sentiments/weekly?startweek=-5&endweek=-1&electorate=" +
          eName;
      } else {
        sentimentSrc =
          process.env.VUE_APP_BASE_URL +
          "/political/sentiments/number?startdate=2022-05-22&enddate=2023-06-30&electorate=" +
          eName;
        lineSrc =
          process.env.VUE_APP_BASE_URL +
          "/political/sentiments/weekly?startweek=1&endweek=5&electorate=" +
          eName;
      }

      const [posResponse, negResponse, senResponse, lineResponse] =
        await Promise.all([
          axios.get(posSrc),
          axios.get(negSrc),
          ...
          () => {
            ...
            map.data.setStyle((feature) => {
              ...
              if (label == "vote") {
                ...
                color = that.gradient("#FFFF00", "#FF0000", 31)[
                  parseInt(vote * 100 - 50)
                ];
              } else if (label == "sentiment") {
                const sentiment =
                  that.output[feature.getProperty("divisionName")][
                    "avg_sentiment"
                  ];
                if (sentiment < 0) {
                  color = that.gradient("#FFFF00", "#0000FF", 81)[
                    parseInt(sentiment * -1 * 100)
                  ];
                } else {
                  color = that.gradient("#FFFF00", "#FF0000", 81)[
                    parseInt(sentiment * 100)
                  ];
            ...
            map.data.addListener("click", async (event) => {
              ...
              this.mapItem.netMigration = event.feature
                ...
                .toFixed(2);
              this.mapItem.winparty19 =
                event.feature.getProperty("winningParty2019");
              this.mapItem.winpartyvote19 = event.feature
                .getProperty("winningPercentage2019")
                .toFixed(2);
              this.mapItem.winparty22 =
                event.feature.getProperty("winningParty2022");
              this.mapItem.winpartyvote22 = event.feature
                .getProperty("winningPercentage2022")
                .toFixed(2);
              this.mapItem.overall_sentiment =
                that.output[event.feature.getProperty("divisionName")][
                  "avg_sentiment"
                ].toFixed(3);
              this.mapItem.tweets_num =
                that.output[event.feature.getProperty("divisionName")][
                  "num_tweets"
                ];
            ...
            map.data.addListener("mouseover", async (event) => {
              ...
              const content =
                event.feature.getProperty("divisionName") + "<br/>" + value;
