        ...
        <el-col class="filter3" :span="4">
          ...
          <el-row>
            <div class="description">
              ...
              <br />
              <div style="margin: 10px; margin-top: 0px; color: #fff">
                {{ description1 }}
              </div>
            </div>
          </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      after: "1w",
      description1:
        "This graph denotes the aggregated National Political Sentiment on Twitter over the period of analysis. You can select levels of time granularity for aggregation. ",
      description2:
        "This graph denotes the sentiment distribution for electorates that changed vs didn’t change governing parties in the recent election. You can select time periods of aggregation relative to the election date.",
    };
  },
  created() {
    this.loadEcharts1();
  ...
  mounted() {
    ...
    this.chartInstance2 = echarts.init(this.$refs.scatterchart);
  ...
  methods: {
    ...
    getData() {
      if (this.interval == "day") {
        return this.$axios
          .get(
            process.env.VUE_APP_BASE_URL +
              "/board/political/sentiments/daily?startdate=2022-02-09&enddate=2023-06-30"
          )
          .then((result) => {
        ...
        return this.$axios
          .get(
            process.env.VUE_APP_BASE_URL +
              "/board/political/sentiments/weekly?startweek=-15&endweek=15"
          )
          .then((result) => {
        ...
        return this.$axios
          .get(
            process.env.VUE_APP_BASE_URL +
              "/board/political/sentiments/monthly?startdate=2022-02-09&enddate=2023-06-30"
          )
          .then((result) => {
    ...
    getChart2Data() {
      ...
      if (this.after == "1w") {
        src =
          process.env.VUE_APP_BASE_URL +
          "/board/political/sentiments/winningchange?type=week&after=1";
      } else if (this.after == "2w") {
        src =
          process.env.VUE_APP_BASE_URL +
          "/board/political/sentiments/winningchange?type=week&after=2";
      } else if (this.after == "1m") {
        src =
          process.env.VUE_APP_BASE_URL +
          "/board/political/sentiments/winningchange?type=month&after=1";
      } else if (this.after == "2m") {
        src =
          process.env.VUE_APP_BASE_URL +
          "/board/political/sentiments/winningchange?type=month&after=2";
      } else {
        src =
          process.env.VUE_APP_BASE_URL +
          "/board/political/sentiments/winningchange?type=month&after=3";
      }
      return this.$axios.get(src).then((result) => {
        let chartData = [];
